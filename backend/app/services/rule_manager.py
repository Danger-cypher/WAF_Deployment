...
def _atomic_write(path: str, content: str) -> None:
    """Write atomically (temp file + rename on the same filesystem) so a
    crash mid-write can never leave a truncated/corrupt config file on
    disk. Shared by both generated-config writers below."""
    directory = os.path.dirname(path)
    os.makedirs(directory, exist_ok=True)
    fd, tmp_path = tempfile.mkstemp(dir=directory, prefix=".modsec-gen-", suffix=".tmp")
    try:
...
def _ensure_include(main_conf_path: str, include_line: str, before_marker: Optional[str] = None) -> None:
    """Ensures `include_line` is Included in main.conf, exactly once.

    `before_marker`, when given and present, inserts the include right
    before that line rather than appending to the end of the file — load
    order inside a ModSecurity phase matters: a `ctl:ruleRemoveById`
    action only suppresses a same-phase target rule if it *executes*
    before that rule would otherwise run, which means the file defining
    it must be Included earlier than the target rule's own file. Simply
    appending (the old behavior) always placed new includes at the very
    end, which is correct for a phase-1 SecAction/SecRuleRemoveById that
    only needs to run at all, but silently wrong for an exclusion meant
    to preempt a CRS phase-1 rule (see 2026-09-04 920350 incident).
    """
    ...
    if include_line in data:
        return
    if before_marker and before_marker in data:
        data = data.replace(before_marker, f"{include_line}\n{before_marker}", 1)
    else:
        data = data.rstrip("\n") + f"\n{include_line}\n"
...
def _update_modsecurity_exclusions_before_crs_file() -> Tuple[bool, str]:
    """
    Writes the active custom exclusions into
    /etc/nginx/modsec/rules-exclusions-before-crs.conf — a separate file
    from rules-override.conf, Included BEFORE coreruleset/rules/*.conf in
    main.conf (see _ensure_include's docstring for why).

    Split out from rules-override.conf on 2026-09-04: that file is
    Included *after* CRS's own rule files, which is required for
    SecRuleRemoveById (a config-time directive that must reference an
    already-loaded rule ID) but is exactly backwards for a runtime
    `ctl:ruleRemoveById` exclusion — those need to execute before the
    target rule, not after. An exclusion targeting a phase:2+ CRS rule
    (body/response inspection, e.g. 941390/930120/942100) happened to
    keep working from the old after-CRS position, since all of phase:1
    (wherever this file is Included) always completes before phase:2
    begins — but an exclusion targeting a phase:1 rule (e.g. 920350,
    "numeric IP Host header") silently never suppressed anything: the
    target had already evaluated and logged earlier in the same phase by
    the time the exclusion's ctl action ran. Real incident: an admin's
    own IP got auto-blacklisted after retrain-status polling tripped
    953100 (phase 4 — this file's old position was fine for that one),
    and a 920350 exclusion added to fix the accompanying noise silently
    did nothing until this split.
    """
    exclusions_path = "/etc/nginx/modsec/rules-exclusions-before-crs.conf"
    lines = [
        "# ========================================================",
        "# CyberSentinel WAF GUI Auto-generated Exclusions (pre-CRS)",
        f"# Timestamp: {datetime.now().isoformat()}",
        "# Do NOT edit this file manually. Changes will be overwritten.",
        "#",
        "# Included BEFORE coreruleset/rules/*.conf — required so a",
        "# ctl:ruleRemoveById exclusion here actually executes before the",
        "# CRS rule it targets, including phase:1 targets. See",
        "# rule_manager._update_modsecurity_exclusions_before_crs_file for",
        "# the full explanation.",
        "# ========================================================",
        "",
        "# --- Active Custom Exclusions & Exceptions ---",
    ]
    ...
    try:
        ...
        logger.error(f"Error loading custom exclusions for pre-CRS exclusions file: {e}")

    content = "\n".join(lines) + "\n"

    try:
        _atomic_write(exclusions_path, content)
        _ensure_include(
            "/etc/nginx/modsec/main.conf",
            f"Include {exclusions_path}",
            before_marker="Include /etc/nginx/modsec/coreruleset/rules/*.conf",
        )
        return True, "Pre-CRS exclusions configuration written successfully."
    except Exception as e:
        logger.error(f"Failed to write pre-CRS exclusions file at {exclusions_path}: {e}")
        return False, f"Could not write WAF pre-CRS exclusions configuration to disk: {e}"
...
def _update_modsecurity_override_file(
    ...
    (paranoia level, anomaly thresholds, and SecRuleRemoveById disabled-
    rule directives — all fine to stay in their existing after-CRS
    position; SecRuleRemoveById in particular must load after CRS since
    it's a config-time directive removing an already-defined rule ID).

    The "Active Custom Exclusions" section used to also live in this file
    — moved to _update_modsecurity_exclusions_before_crs_file() instead,
    see that function's docstring for why. Also writes that file on every
    call so the two configs can never drift out of sync with each other.

    Returns (False, reason) whenever either write genuinely fails —
    callers rely on this to decide whether to roll back and whether to
    tell the admin their change was NOT applied, so this must never
    report success on a failed write.
    """
    override_path = "/etc/nginx/modsec/rules-override.conf"

    # Readiness-review finding: the dashboard's "SecRuleEngine Posture"
    # control (Settings -> WAF Engine Policies) saved to settings.json but
    # was never applied anywhere — modsecurity.conf's own `SecRuleEngine On`
    # was the only value that ever took effect, regardless of what the UI
    # showed. Fetched here (not threaded through every one of this
    # function's callers) so every existing call site — set_paranoia_level,
    # reset_rules, sync_rules_and_exclusions — picks up the current value
    # automatically instead of one of them accidentally reverting it to the
    # default. This file loads after modsecurity.conf's static `SecRuleEngine
    # On` (see main.conf's Include order), so this directive is the one that
    # actually wins — same override-by-later-directive mechanism already
    # relied on for tx.detection_paranoia_level and the anomaly thresholds
    # below. Only On/DetectionOnly/Off are valid; anything else falls back
    # to On rather than silently disabling the engine on a bad value.
    try:
        from app.services.settings_manager import settings_manager
        sec_rule_engine = settings_manager.get_waf_settings().get("secRuleEngine", "On")
    except Exception:
        sec_rule_engine = "On"
    if sec_rule_engine not in ("On", "DetectionOnly", "Off"):
        sec_rule_engine = "On"
    ...
    lines = [
        ...
        "# ========================================================",
        "",
        "# --- SecRuleEngine Posture (Settings -> WAF Engine Policies) ---",
        "# Overrides modsecurity.conf's static `SecRuleEngine On` — this file",
        "# loads later, so this value wins for the life of the config.",
        f"SecRuleEngine {sec_rule_engine}",
        "",
    ...
    try:
        ...
        # Ensure rules-override.conf is included in main.conf (unchanged
        # position — appended at the end is fine here, this file only
        # needs to load after CRS, not before).
        _ensure_include("/etc/nginx/modsec/main.conf", f"Include {override_path}")

        # Keep the pre-CRS exclusions file in lock-step — a toggle_rule/
        # set_paranoia_level/reset_rules/sync_rules_and_exclusions call
        # only invokes this one function, so without this the two files
        # could report success while one silently kept stale content.
        exc_ok, exc_msg = _update_modsecurity_exclusions_before_crs_file()
        if not exc_ok:
...
def sync_rules_and_exclusions() -> Tuple[bool, str]:
    ...
    # Readiness-review finding: this used settings_manager.get("waf", {}),
    # but SettingsManager has no generic get() — every call raised
    # AttributeError, silently swallowed by the except below, so these two
    # thresholds have always used the hardcoded fallback (5, 4) regardless
    # of what was actually saved. get_waf_settings() is the real accessor.
    try:
        from app.services.settings_manager import settings_manager
        waf_cfg = settings_manager.get_waf_settings()
...
def generate_modsec_rule(
    ...
    # 6. Host-header specific exclusion — for a vhost that's permanently
    # accessed via a raw IP with no DNS name (e.g. this admin dashboard on
    # :3020), where CRS's generic "numeric IP Host header" rule (920350) is
    # a permanent, expected false positive rather than a real bot/scanner
    # signal. Scoped to the exact Host value (reuses the `uri` field to
    # carry it — no schema change needed) so every other vhost, including
    # customer-protected apps where a raw-IP Host header IS still a real
    # signal, keeps full protection from the excluded rule.
    elif exclusion_type == "host":
        if not uri:
            raise ValueError("Host value is required for host-specific exclusions.")
        return f'SecRule REQUEST_HEADERS:Host "@streq {uri}" "id:{rule_num_id},phase:1,pass,nolog,ctl:ruleRemoveById={rule_id}"'
    else:
