...
# Posture" toggle saved to settings.json but nothing ever applied it —
# rule_manager._update_modsecurity_override_file() now writes the real
# ModSecurity SecRuleEngine directive from it, but that alone turned out not
# to be enough: this deployment's actual block/allow decision is made by
# ml_decide.lua reading the CRS score independently of ModSecurity's own
# disruptive-action mechanism (SecRuleEngine DetectionOnly only suppresses
# THAT, and this stack disables it entirely in favor of the Lua layer — see
# ml_check.lua / ml_decide.lua's own header comments). So Lua needs to know
# the posture too. Deliberately global (conf.d/*.conf, not a per-app opt-in
# file like HARDENING_CONF_PATH above) — this is a global admin setting.
WAF_ENGINE_POSTURE_CONF_PATH = "/etc/nginx/conf.d/waf_engine_posture.conf"
...
def apply_waf_engine_posture(sec_rule_engine: str) -> tuple[bool, str]:
    """
    Exposes Settings -> WAF Engine Policies' "SecRuleEngine Posture" to the
    Lua layer as $waf_sec_rule_engine_posture, using the exact same
    map-a-constant trick apply_ddos_settings() already uses for
    $waf_risk_challenge_enabled etc. ("map $host $var { default X; }" —
    $host is ignored, this just injects a Python-decided constant into the
    request-variable namespace Lua can read).

    ml_decide.lua checks this before its own CRS-score/ML-engine block
    decisions and skips them entirely when the posture isn't "On" — see that
    file's own comment for why this is necessary at all (ModSecurity's real
    SecRuleEngine directive, applied by rule_manager, doesn't reach this
    stack's actual enforcement path, which is Lua-driven, not ModSecurity's
    native disruptive actions).
    """
    if sec_rule_engine not in ("On", "DetectionOnly", "Off"):
        sec_rule_engine = "On"
    config_content = "\n".join([
        "# Auto-generated by CyberSentinel WAF GUI — do not edit manually.",
        "# See nginx_manager.apply_waf_engine_posture()'s docstring for why",
        "# this exists (readiness-review finding: the dashboard toggle used",
        "# to save this value without applying it anywhere real).",
        "map $host $waf_sec_rule_engine_posture {",
        f'    default "{sec_rule_engine}";',
        "}",
        "",
    ])
    return write_and_apply_configs({WAF_ENGINE_POSTURE_CONF_PATH: config_content})
